...
bool iscross(point p1,point p2,point p3,point p4){
	//判断两线段ab与cd是否相交，两个实验：快排实验和跨立实验
	if(min(p1.x,p2.x)>max(p3.x,p4.x)||min(p3.x,p4.x)>max(p1.x,p2.x)
	||min(p1.y,p2.y)>max(p3.y,p4.y)||min(p3.y,p4.y)>max(p1.y,p2.y))	return 0;
	//快排实验：如果一条线段两端点的横（纵）坐标均大于另一条线段两端点的横（纵）坐标
	//那么这两条线段不可能相交，返回false 
	...
	return 0;
	//跨立实验：如果两条线段相交，那么他们肯定互相跨越彼此
	//我们先判断一条线段的两个端点是否在另一条线段的两侧
	//然后再对另一条线段这么做，如果至少有一条线段不能跨越另一条线段，返回false
	//否则返回true 
}
